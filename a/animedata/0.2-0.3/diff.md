# Comparing `tmp/animedata-0.2.tar.gz` & `tmp/animedata-0.3.tar.gz`

## Comparing `animedata-0.2.tar` & `animedata-0.3.tar`

### file list

```diff
@@ -1,8 +1,8 @@
--rw-r--r--   0        0        0      400 2020-02-02 00:00:00.000000 animedata-0.2/ad_test.py
--rw-r--r--   0        0        0     4906 2020-02-02 00:00:00.000000 animedata-0.2/animedata.py
--rw-r--r--   0        0        0    17646 2020-02-02 00:00:00.000000 animedata-0.2/animedata_source.json
--rw-r--r--   0        0        0       44 2020-02-02 00:00:00.000000 animedata-0.2/.gitignore
--rw-r--r--   0        0        0     1086 2020-02-02 00:00:00.000000 animedata-0.2/LICENSE
--rw-r--r--   0        0        0     1874 2020-02-02 00:00:00.000000 animedata-0.2/README.md
--rw-r--r--   0        0        0      701 2020-02-02 00:00:00.000000 animedata-0.2/pyproject.toml
--rw-r--r--   0        0        0     2371 2020-02-02 00:00:00.000000 animedata-0.2/PKG-INFO
+-rw-r--r--   0        0        0      597 2020-02-02 00:00:00.000000 animedata-0.3/ad_test.py
+-rw-r--r--   0        0        0     4860 2020-02-02 00:00:00.000000 animedata-0.3/animedata.py
+-rw-r--r--   0        0        0    18086 2020-02-02 00:00:00.000000 animedata-0.3/animedata_source.json
+-rw-r--r--   0        0        0       44 2020-02-02 00:00:00.000000 animedata-0.3/.gitignore
+-rw-r--r--   0        0        0     1086 2020-02-02 00:00:00.000000 animedata-0.3/LICENSE
+-rw-r--r--   0        0        0     1874 2020-02-02 00:00:00.000000 animedata-0.3/README.md
+-rw-r--r--   0        0        0      701 2020-02-02 00:00:00.000000 animedata-0.3/pyproject.toml
+-rw-r--r--   0        0        0     2371 2020-02-02 00:00:00.000000 animedata-0.3/PKG-INFO
```

### Comparing `animedata-0.2/animedata.py` & `animedata-0.3/animedata.py`

 * *Files 12% similar despite different names*

```diff
@@ -7,24 +7,24 @@
 import warnings
 import os.path
 from copy import deepcopy
 
 dir_path = os.path.dirname(__file__)
 ad_table = {
     "repository_url":
-    f"https://raw.githubusercontent.com/swarthur/animedata/",
+    "https://raw.githubusercontent.com/swarthur/animedata/",
     "source_file_name": "./animedata_source.json",
     "local_file_name": "./animedata_local.json",
-    "key_anime_name": "anime_name",
-    "key_seasons_episodes": "seasons_episodes",
-    "key_episode_duration": "episode_duration",
-    "key_episode_release_date": "episode_release_date",
-    "key_episode_name": "episode_name"}
+    "anime_name": "anime_name",
+    "seasons": "seasons",
+    "episode_duration": "episode_duration",
+    "episode_release_date": "episode_release_date",
+    "episode_name": "episode_name"}
 
-with open(os.path.join(dir_path, "./pyproject.toml"), mode="rb") as pypr:
+with open(os.path.join(dir_path, ".\\pyproject.toml"), mode="rb") as pypr:
     ad_version = tomllib.load(pypr)["project"]["version"]
 print("AnimeData script version : ", ad_version)
 
 
 def get_ad_lib(branch: str = "main"):
     """Download and replace local AnimeData library from Github.
 
@@ -121,15 +121,15 @@
     corrupted_keys = []
     dict_valid = True
     correct_dict = deepcopy(anime_dict)
     for element in anime_dict.keys():
         dict_element = anime_dict[element]
         try:
             if dict_element["type"] == "anime":
-                if dict_element.get(ad_table["key_anime_name"]) != element:
+                if dict_element[ad_table["anime_name"]] != element:
                     corrupted_keys.append(element)
             elif dict_element["type"] == "metadata":
                 corrupted_keys.append(element)
             else:
                 corrupted_keys.append(element)
         except KeyError:
             corrupted_keys.append(element)
```

### Comparing `animedata-0.2/LICENSE` & `animedata-0.3/LICENSE`

 * *Files identical despite different names*

### Comparing `animedata-0.2/README.md` & `animedata-0.3/README.md`

 * *Files identical despite different names*

### Comparing `animedata-0.2/pyproject.toml` & `animedata-0.3/pyproject.toml`

 * *Files 0% similar despite different names*

```diff
@@ -6,15 +6,15 @@
 00000050: 746f 6f6c 2e68 6174 6368 2e62 7569 6c64  tool.hatch.build
 00000060: 5d0d 0a65 7863 6c75 6465 203d 205b 0d0a  ]..exclude = [..
 00000070: 2020 222f 6469 7374 222c 0d0a 2020 222f    "/dist",..  "/
 00000080: 2e67 6974 222c 0d0a 2020 222e 6769 7469  .git",..  ".giti
 00000090: 676e 6f72 6522 0d0a 2020 5d0d 0a0d 0a5b  gnore"..  ]....[
 000000a0: 7072 6f6a 6563 745d 0d0a 6e61 6d65 203d  project]..name =
 000000b0: 2022 616e 696d 6564 6174 6122 0d0a 7665   "animedata"..ve
-000000c0: 7273 696f 6e20 3d20 2230 2e32 220d 0a61  rsion = "0.2"..a
+000000c0: 7273 696f 6e20 3d20 2230 2e33 220d 0a61  rsion = "0.3"..a
 000000d0: 7574 686f 7273 203d 205b 0d0a 207b 6e61  uthors = [.. {na
 000000e0: 6d65 3d22 7377 6172 7468 7572 227d 0d0a  me="swarthur"}..
 000000f0: 5d0d 0a64 6573 6372 6970 7469 6f6e 203d  ]..description =
 00000100: 2022 4f70 656e 2d73 6f75 7263 6520 6c69   "Open-source li
 00000110: 6272 6172 7920 636f 6e74 6169 6e69 6e67  brary containing
 00000120: 2064 6174 6162 6173 6520 6162 6f75 7420   database about 
 00000130: 616e 696d 6573 2f73 6572 6965 7320 616e  animes/series an
```

### Comparing `animedata-0.2/PKG-INFO` & `animedata-0.3/PKG-INFO`

 * *Files 2% similar despite different names*

```diff
@@ -1,10 +1,10 @@
 Metadata-Version: 2.1
 Name: animedata
-Version: 0.2
+Version: 0.3
 Summary: Open-source library containing database about animes/series and including a built-in utility.
 Project-URL: Github, https://github.com/swarthur/animedata
 Project-URL: Bug, https://github.com/swarthur/animedata/issues
 Author: swarthur
 License-Expression: MIT
 License-File: LICENSE
 Classifier: License :: OSI Approved :: MIT License
```

